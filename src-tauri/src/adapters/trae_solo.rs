// Purpose: Trae Solo adapter entry and runtime capability boundary.
...
pub fn create(home: &Path) -> Box<dyn ToolAdapter> {
    let definition =
        registry::definition("trae-solo").expect("missing Trae Solo catalog definition");
    Box::new(
        ...
        .with_skills_dir(home.join(".trae/skills"))
        .with_detection_paths(&[
...
fn capabilities(home: &Path) -> Vec<DeclaredCapability> {
    vec![
        capability(
            ...
            "Trae Solo shares the ~/.trae local data root for user Rules and Skills. MCP and Settings sources are declared separately.",
            ToolCapabilitySourceKind::PrimaryConfigDirectory,
            runtime_action_gates(
                "Trae Solo primary configuration directory",
                "~/.trae",
                &[
        ...
        capability(
            ...
            ToolCapabilityFormat::Directory,
            home.join(".trae/user_rules"),
            "User Rules",
            directory_diagnostics(),
            "Trae Solo local user Rules persist under ~/.trae/user_rules. This directory is a native rule source, not a default Global Rule injection target.",
            ToolCapabilitySourceKind::FeatureSource,
            runtime_action_gates(
                "Trae Solo user Rules directory",
                "~/.trae/user_rules",
                &[
                    ToolCapabilityAction::View,
                    ToolCapabilityAction::Read,
        ...
        capability(
            ...
            "Skills",
            directory_diagnostics(),
            "Trae Solo discovers user-level Skills from ~/.trae/skills. Modus adopts only this tool-specific Skill directory and does not write marketplace metadata or declare shared ~/.agents/skills direct-read.",
            ToolCapabilitySourceKind::FeatureSource,
            runtime_action_gates(
                "Trae Solo Skills directory",
                "~/.trae/skills",
                &[
                    ToolCapabilityAction::View,
                    ToolCapabilityAction::Read,
        ...
        capability(
            ...
            home.join("Library/Application Support/TRAE SOLO/User/mcp.json#servers"),
            "MCP configuration",
            file_diagnostics(),
            "Trae Solo user-level MCP configuration is stored in the app support mcp.json file under servers. Modus edits single entry fragments only and does not manage server lifecycle.",
            ToolCapabilitySourceKind::FeatureSource,
            runtime_action_gates(
                "Trae Solo MCP configuration",
                "~/Library/Application Support/TRAE SOLO/User/mcp.json#servers",
                &[
                    ToolCapabilityAction::View,
                    ToolCapabilityAction::Read,
                    ToolCapabilityAction::Diagnose,
        ...
        capability(
            ...
            ToolCapabilityAccess::Writable,
            ToolCapabilityFormat::Json,
            home.join("Library/Application Support/TRAE SOLO/User/settings.json"),
            "Settings",
            file_diagnostics(),
            "Trae Solo user settings file. Rules, Skills, MCP, caches, logs, and runtime state remain separate feature sources.",
            ToolCapabilitySourceKind::FeatureSource,
            runtime_action_gates(
                "Trae Solo settings",
...
fn capability(
    ...
    DeclaredCapability {
        ...
        notes,
        source_kind,
        primary_config_dir: Some(home.join(".trae").to_string_lossy().to_string()),
...
mod tests {
    ...
    #[test]
    fn runtime_capabilities_adopt_only_tool_skill_directory() {
        let adapter = create(Path::new("/Users/example"));
        let capabilities = adapter.capabilities();

        assert_eq!(
            adapter.config_dir(),
            Path::new("/Users/example").join(".trae")
        );
        assert_eq!(
        ...
        let rule_projections =
            project_capabilities("trae-solo", ToolCapabilityModule::Rules, &capabilities);
        assert!(rule_projections.iter().any(|projection| {
            projection.source_role == ToolCapabilitySourceRole::RuleNativeFileSource
                ...
                && projection.evidence.source_path == "/Users/example/.trae/user_rules"
        }));

        let skill_projections =
            project_capabilities("trae-solo", ToolCapabilityModule::Skills, &capabilities);
        assert!(skill_projections.iter().any(|projection| {
            projection.source_role == ToolCapabilitySourceRole::SkillToolDirectory
                ...
                && projection.allows(&ProjectedAction::Link)
                && projection.evidence.source_path == "/Users/example/.trae/skills"
        }));
        assert!(!skill_projections.iter().any(|projection| {
        ...
        let mcp_projections =
            project_capabilities("trae-solo", ToolCapabilityModule::Mcp, &capabilities);
        assert!(mcp_projections.iter().any(|projection| {
            projection.allows(&ProjectedAction::Save)
                && projection.evidence.source_path
                    == "/Users/example/Library/Application Support/TRAE SOLO/User/mcp.json#servers"
        }));
        ...
        assert!(ordinary_projections.iter().any(|projection| {
            projection.allows(&ProjectedAction::Save)
                && projection.evidence.source_path
                    == "/Users/example/Library/Application Support/TRAE SOLO/User/settings.json"
        }));
    }
}
